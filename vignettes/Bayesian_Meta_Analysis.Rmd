...
This is a useful function because it performs very quickly despite the need for a high number of simulations in order to attain stable estimates. Below we see an
...
marginalLikelihood(as.matrix(data_ex[,2:5]), hats$par[1], hats$par[2], 10000, 0, 100)
```
...
posteriors <- mhGibbs(data_ex, hyperparameters_ex, 0.2, sim = 100000, burn = 1000)$thetas |>
    as.data.frame()

names(posteriors)<- c("Balcon",
                 "Clausen",
                  "Multicentre",
                   "Barber",
                   "Norris",
...
posteriors <- stack(posteriors)
names(posteriors) <- c("Est", "Trial")
